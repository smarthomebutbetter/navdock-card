...
class NavDockCard extends HTMLElement {
  ...
  _render() {
    ...
    const _ndMob = window.innerWidth < (Number(this._config.breakpoint) || 768);
    const maxWidth = Math.max(260, Number(this._config.width) || (_ndMob ? 336 : 520));
    const panelWidth = Math.max(300, Number(this._config.panel_width) || maxWidth);
    const bottom = Math.max(4, Number(this._config.bottom) || (_ndMob ? 28 : 18));
    ...
    const height = Math.max(56, Math.min(96, Number(this._config.height) || (_ndMob ? 78 : 68)));
    ...
    const pillW = Number(this._config.active_pill_width) > 0 ? `${Number(this._config.active_pill_width)}px` : '100%';
    const pillH = Number(this._config.active_pill_height) > 0 ? `${Number(this._config.active_pill_height)}px` : '100%';
    const pillR = Number(this._config.active_pill_radius) >= 0 ? `${Number(this._config.active_pill_radius)}px` : '999px';
    ...
    const accent = getComputedStyle(this).getPropertyValue('--primary-color').trim() || '#5b67b3';
    ...
    this.shadowRoot.innerHTML = `
      ...
      <section class="stack ${preview ? 'preview' : ''} ${isMobile ? 'mobile' : 'desktop'} ${isMobileSheet ? 'mobile-sheet' : ''} ${placement === 'docked' ? 'docked' : 'floating'} ${showLabels ? '' : 'hide-labels'} ${this._config.shadow === false ? 'no-shadow' : ''} ${closingClass}" style="--media-offset:${mediaOffset}px;--nd-height:${height}px;--nd-radius:${radius};--nd-icon-size:${iconSize}px;--nd-label-size:${labelSize}px;--nd-accent:${accent};--nd-max-width:${maxWidth}px;--nd-panel-width:${panelWidth}px;--nd-shadow:${shadowStyle};--nd-dock-bg:${dockBg};--nd-dock-border:${dockBorder};--nd-dock-shadow:${dockShadow};--nd-pill-w:${pillW};--nd-pill-h:${pillH};--nd-pill-r:${pillR}">
        ${!preview && mediaVisible && !this._expanded && !this._profileOpen ? this._compactTemplate(media) : ''}
        ${'<nav class="dock" aria-label="Dashboard Navigation">'+tabs.map((tab, index) => this._tabTemplate(tab, index)).join('')+(profileEnabled ? this._profileTemplate() : '')+'</nav>'}
      </section>`;
  ...
  _styles(maxWidth, bottom) {
    return `
      ...
      *, *::before, *::after { box-sizing:border-box; }
      ...
      .expanded.mobile-sheet,.profile-panel.mobile-sheet{position:fixed;top:calc(env(safe-area-inset-top) + 10px);bottom:calc(${bottom}px + env(safe-area-inset-bottom) + var(--nd-height) + 8px);left:50%;transform:translateX(-50%);width:calc(100vw - 20px);max-height:none;border:1px solid var(--nd-border);box-shadow:var(--nd-shadow,0 0 0 1px rgba(255,255,255,.075),0 14px 42px rgba(0,0,0,.38),0 3px 10px rgba(0,0,0,.24));border-radius:24px;padding:16px;overflow:auto;z-index:101}
      ...
      .expanded:not(.mobile-sheet),.profile-panel:not(.mobile-sheet){position:fixed;left:50%;transform:translateX(-50%);max-width:420px;max-height:70vh;bottom:calc(var(--nd-height) + 24px)}
      ...
      .tab { position:relative; min-width:0; flex:1 1 0; height:calc(var(--nd-height) - 12px); padding:5px 4px; border:0; border-radius:999px; color:var(--secondary-text-color); background:transparent; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px; cursor:pointer; transition:transform .18s ease,background .18s ease,color .18s ease; z-index:1; }
      .tab:active { transform:scale(.94); }
      .tab.active { color:var(--nd-accent); background:transparent; }
      .tab.active::after { content:''; position:absolute; left:50%; top:50%; transform:translate(-50%,-50%); width:var(--nd-pill-w); height:var(--nd-pill-h); border-radius:var(--nd-pill-r); background:var(--nd-surface-soft); box-shadow:inset 0 0 0 1px var(--nd-border); z-index:-1; transition:width .18s ease,height .18s ease,border-radius .18s ease; }
      ...
      .compact { min-height:var(--nd-height); border-radius:var(--nd-radius); padding:7px 8px; display:grid; grid-template-columns:50px minmax(0,1fr) auto; align-items:center; gap:10px; cursor:pointer; animation:nd-pop .16s ease-out; }
      ...
      .expanded { width:min(calc(100vw - 24px),var(--nd-panel-width));min-height:390px;align-self:center;border-radius:var(--nd-radius); padding:24px; animation:nd-pop .16s ease-out; transform-origin:bottom center;display:flex;flex-direction:column;justify-content:space-between;touch-action:pan-y; }
      .expanded [data-collapse]{position:absolute;top:14px;right:14px;z-index:10}
      ...
      @keyframes nd-pop{from{opacity:0}to{opacity:1}}
